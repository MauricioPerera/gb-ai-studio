...
if (!fm) { add('error', 'frontmatter-present', 'Falta el front-matter YAML (--- ... ---).'); }
...
// palette-range (platform + indices de paleta declarados)
...
// palette-color-range: cada color es [r,g,b] en 0..31
...
// solid-sync: cruzar tiles.solid con GBA_SOLID_TILES en simulator.js
let solidSet = null;
try {
  const sim = fs.readFileSync(path.join(root, 'simulator.js'), 'utf8');
  const m = sim.match(/GBA_SOLID_TILES\s*=\s*new Set\([^[]*\[([^\]]+)\]/);
  if (m) solidSet = new Set(m[1].split(',').map(s => parseInt(s.trim())).filter(n => !isNaN(n)));
} catch (e) { /* simulator.js no encontrado */ }
if (solidSet) {
  for (const [id, t] of Object.entries(tiles)) {
...
// type-symmetry (la diagonal a===a queda fuera: la resistencia al propio tipo es válida)
...
// item-effect-valid: efecto conocido; heal requiere amount; cure requiere estado
...
// encounter-ref: cada especie de la tabla de encuentros por area debe existir
...
// map-dims / map-legend-ref: dimensiones segun platform y simbolos que referencian tiles/paletas validos
...
for (const [name, def] of Object.entries(data.maps || {})) {
  ...
  // map-meta: entry/exit/return en rango; la salida debe caer sobre un felpudo (tile 46)
...
// overworld-ref: entidades por area (NPCs + colocacion de entrenadores + warps)
...
// tileart-ref / tileart-dims: arte de tiles 8x8 con índices de color en rango y IDs conocidos
...
// text-valid: cada texto de sistema es una cadena no vacía (y sin comillas dobles internas sin escapar)
...
// player-ref: estado inicial coherente
...
// economy-bounds: precios (desde items) y rango de captura
...
// dead-token: claves de `balance` declaradas en GAME.md pero nunca referenciadas en el motor.
// Cubre el patron gBal('x'), acceso .x y ['x'] (asi se evita el drift que rompio encounterRate/potionHeal).
try {
  const sim = fs.readFileSync(path.join(root, 'simulator.js'), 'utf8');
  for (const k of Object.keys(bal)) {
    const e = k.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const used = new RegExp("gBal\\(['\"]" + e + "['\"]|\\." + e + "\\b|\\[['\"]" + e + "['\"]\\]").test(sim);
    if (!used) add('warn', 'dead-token', 'balance.' + k + ' declarado en GAME.md pero no referenciado en simulator.js');
  }
} catch (e) { /* simulator.js no encontrado: cross-check omitido */ }
